...
There is a particular kind of madness that comes from staring at a browser console full of red text that reads `Access-Control-Allow-Origin`. It's the kind of madness that makes you question your career choices, your framework choices, and eventually — at 5:23 PM on a Friday — your choice to use cookies at all.
...
While CORS held center stage, authentication was waging its own quiet campaign of destruction behind the curtain. Allow me a brief digression, because the auth bugs and the CORS bugs were often indistinguishable from each other in the browser console.
...
The fix: an `X-Internal-Key` header bypass. If you carry the internal key, you skip auth entirely. Simple, practical, and the kind of thing you only build after watching a service-to-service call fail in production.
...
The `Vary: Origin` header tells caches that responses differ by origin — critical when you're reflecting, or you'll serve one origin's CORS headers to another. This is the detail that separates a reflected-origin implementation from a security vulnerability.
...
## What We Learned (Written in Scar Tissue)
...
### 3. Reflected origin is the endgame

For internal gateways where you control all the clients, reflecting the `Origin` header back is the simplest correct solution. Set `Vary: Origin` so caches behave, and you're done. No list to maintain. No deployments to remember.
...
It routes 16 services. It handles three auth strategies. It rate-limits at different tiers per route type. It caches with organization-scoped keys. And its CORS middleware is 23 lines of reflected-origin simplicity that hasn't needed a change since March 21st.

Sometimes the battle is the architecture.

---

*Questions about our gateway setup? Battle scars from your own CORS wars? Check out our repos and come commiserate.*
